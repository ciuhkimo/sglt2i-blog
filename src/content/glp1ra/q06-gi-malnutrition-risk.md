...
tags:
  ...
  - decision-note
  - safety
description: "CKD/frail 病人使用 GLP-1 RA 時的 GI 副作用管理與營養不良風險。涵蓋 GI→脫水→AKI 路徑、lean mass 流失疑慮、PEW 風險分層、分藥物腎臟用法、2024 更新版術前停藥指引、實務監測與暫停規則。"
quick_answer: "GI 副作用是 GLP-1 RA 最常見的耐受性問題，在 CKD/frail 病人風險更高（GI→脫水→AKI、營養惡化）。肥胖且營養穩定的 CKD 病人可管理；低 BMI/PEW/frailty 者一般不宜啟用。劑量策略以最低有效且可耐受為目標，非機械式上調。術前停藥已更新為個別化風險分層。"
seo_title: "CKD 病人用 GLP-1 RA 的 GI 副作用與營養風險：實務管理指引"
---
...
1. **處方前進行營養風險分層**。根據衰弱表型決定 GLP-1 RA 適用性：肥胖型 CKD（含肌少型肥胖 SO，BMI >=25）為推薦族群，效益明確且 GI 可管理，但仍需搭配蛋白質攝取與阻力訓練。

2. **正常體重 CKD、邊緣營養者（BMI 23-25、albumin 3.5-4.0）可使用但需極度謹慎**。必須同步轉介營養師、延長調升間隔、密集監測。

3. **低 BMI / 活性 PEW / 厭食-營養不良型衰弱（AM）者一般不宜啟用 GLP-1 RA**。BMI <23、albumin <3.5、近期非預期減重、CFS >=5/Fried 陽性、HbA1c 自然下降者屬此類。進一步食慾抑制與體重下降將加速功能崩壞，可考慮 SGLT2i（GI 副作用極低）為替代。

4. **CKD 患者優先選擇 semaglutide SC 或 dulaglutide**。Exenatide ER（eGFR <45 不建議）和 lixisenatide（eGFR <15 不建議）有腎功能門檻限制。

5. **所有 CKD 4-5D、albumin <3.5、BMI <23、限蛋白飲食者處方前應轉介營養師**。

6. **劑量以最低有效且可耐受為目標，非機械式上調至最大劑量**。0.5 mg 若已達足夠效果，可維持而不必升至 1.0 mg。

### 不建議做法

1. **對低 BMI / PEW / AM 型衰弱者啟用 GLP-1 RA**，除非肥胖表型確認且營養穩定。

2. **使用標準 4 週調升間隔於高風險 CKD 患者**。CKD 改良方案建議 6-8 週，高風險者 8-12 週。

3. **忽略基線 GI 症狀評估**。CKD 患者基線 GI 盛行率已高，已有胃輕癱者 FDA 不建議使用。

4. **將營養監測閾值視為經驗證的證據**。本文所列閾值（albumin <3.0 暫停、減重 >5%/月暫停等）均為專家衍生的實務操作門檻，未經前瞻性研究驗證。
...
1. **GLP-1 RA 是否真正加速 CKD 患者的 PEW/肌少症**：無 RCT 以此為終點
2. **最佳營養監測閾值**：本文所列閾值為專家衍生的實務操作門檻，未經前瞻性研究驗證
3. **CKD 限蛋白飲食 + GLP-1 RA 食慾抑制的交互作用**：完全未被研究
4. **非糖尿病 CKD 的長期 GI 安全性**：僅 SMART（n=101, 24 wk）
5. **最佳復藥策略**：完全缺乏 CKD 特異性實證
6. **瘦體組織流失在 CKD 的具體比例**：肥胖族群 RCT 報告 15-40%，CKD 特異性數據缺乏
7. **衰弱 CKD 患者的風險-效益比**：無法從現有試驗族群（BMI ~32）外推
