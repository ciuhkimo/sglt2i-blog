...
tags:
  ...
  - Taiwan
  - decision-note
seo_title: "ARR 切點與藥物洗脫：PA 篩檢怎麼驗才不會驗錯｜腎臟科決策筆記"
description: "原發性醛固酮症（PA）篩檢的 ARR 判讀：以 PRA 計算與以 DRC 計算的切點不可互換、PAC 絕對下限不可省；藥物洗脫三檔策略（停 4 週／2 週／不停藥）、橋接用藥、抽血前 K⁺ 與姿勢條件，以及 CKD／老年／已用 MRA／孕婦特殊族群判讀。區域醫院腎臟科案頭速查。"
quick_answer: "以 PRA 計算的 ARR 與以 DRC 計算的 ARR 不可互換、切點也不同（PRA-based 主力 ARR ≥30；2025 Endocrine Society 以免疫分析法 >20 為起點），而且必須同時 PAC ≥10–15 ng/dL——ARR 高但 PAC 不高是低腎素假象、不是 PA。藥物洗脫：MRA／ENaC 抑制劑／利尿劑停 4 週，β-blocker／ACEi/ARB／DHP-CCB／中樞 α2-agonist／SGLT2i 停 2 週，橋接用 verapamil SR／doxazosin／hydralazine；2025 ES 允許『先不停藥也可驗』以降低門檻（非無條件免停藥，陰性且中高風險仍須停藥重驗）。抽血前先把 K⁺ 補到 ≥4.0、晨間坐姿 5–15 分鐘、不限鹽。"
---
...
```mermaid
flowchart TD
    A[擬驗 ARR] --> B{高度懷疑?<br/>自發低 K⁺ / 頑固高血壓}
    B -- 是 --> C[先不停藥直接驗]
    B -- 一般疑似 --> D[評估現有藥物干擾 §3]
    D --> E[必要時換橋接<br/>verapamil SR / doxazosin ± hydralazine]
    C --> F[抽血前: K⁺≥4.0 / 晨間坐姿 5-15 分 / 自由鹽]
    E --> F
    F --> G{判讀}
    G -- ARR↑ 且 PAC 達 assay 切點<br/>免疫法≥10 / LC-MS/MS≥7.5 --> H[陽性]
    G -- ARR↑ 但 PAC低 --> I[低腎素假象 → 重評/重抽]
    G -- ARR陰性 但仍高度懷疑 --> J[校正 K⁺ / 停藥 → 擇日重驗]
    G -- 邊緣值 --> K[校正 K⁺ + 停藥 → 擇日重驗]
    H --> L{符合免確認條件?<br/>腎素測不到 + 自發低 K⁺<br/>+ PAC>20 免疫分析法 / >15 LC-MS/MS}
    L -- 是 --> M[免確認 → 直接進 Q4 定位]
    L -- 否 --> N[進 Q3 確認試驗]
    I --> G
    J --> G
